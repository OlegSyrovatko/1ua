...
if(Auth::user()){$Numm = Auth::user()->id;} else{$Numm="0000000000";}
 ...
 $Allcm = DB::table('Memorymp')->select('idrec2','Aboutef', 'Md', 'avt')->
 orderBy('Md', 'asc')->
 where('idrec', $M5)->get();
 $nrf2 = $Allcm->count();
 ...
 if ($nrf2 > 0){
     ...
     foreach ($Allcm as $All) {
         ...
         $idrec = $All->idrec2; $M11 = $All->Aboutef; $M6m = $All->Md; $avtc = $All->avt;
         $M6m_e= db_date($M6m);
        ...
        <div id=\"$t3\">
           ...
           if($admpass=="ok"||($avtc==$Numm&&$avtc>10)||$avt==$Numm || $Numm=="72372396"){
               ...
               echo" <a onclick=$t2.style.display='block';>$Delete</a>
                ...
                <table><tr><td align=left width=100><b><a onclick=\"commm_delp('$t3','$idrec'); return false;\">$yesd</a></b></td>
        ...
        echo"</td></tr></table>
           ...
           if(($nrf2>2)&&($hidc_up==2)){
            ...
            <div id=shc$M5$purp>
            <table><tr><td class=fcom style=\"cursor: pointer;\" width=250 align=center onMouseOver=\"this.style.background='white'\" onMouseOut=\"this.style='fcom'\"
            onClick=mem_not_delmp('shc$M5$purp','hc$M5$purp')><b>$see_all $nrf2 $cmts</b></td></tr>
            </table>
            </div>";
           }
     ...
     if($red=="on"){
        ...
        <textarea id=\"$fromdiv\" rows=3 style=\"width: 250px;\"
         onFocus=clearsp('$fromdiv','rbc$M5');>$M11</textarea>
        <div id=\"rbc$M5\" style=\"display: none;\">
            <table><tr><td class=\"fcomblue\" width=100>
                <ul class=\"intop\"><li><a onclick=\"commm_redp('$idrec','$fromdiv','$indiv','$purp')\">$Ed</a></li></ul>
            </td></tr></table>
        </div>
        </div><br />";
    }
