...
$Allcm = DB::table('Memoryf')->select('idrec','Aboutef', 'Md', 'avt')->
orderBy('Md', 'asc')->
where('Num', $M5)->get();
$nrf2 = $Allcm->count();
...
if ($nrf2 > 0){
    ...
    foreach ($Allcm as $All) {
        ...
        $M11 = $All->Aboutef; $idrec = $All->idrec; $M6m = $All->Md; $avtc = $All->avt;
        ...
        <div id=\"$t3\" class=\"mt5\" style\"max-width: 400px; \">
           ...
           if($admpass=="ok"||($avtc==$Numm&&$avtc>10)||$avt==$Numm || $Numm=="72372396"){
               echo"<a onclick=$t2.style.display='block';>" . __('messages.Delete') . "</a>
                ...
                <table><tr><td align=left width=100><b><a onclick=\"comm_del('$t3','$idrec'); return false;\">" . __('messages.yes'). "</a></b></td>
        ...
        if(($nrf2>2)&&($hidc_up==2)){
           echo"</div>
            <div id=shc$M5 class=\"fcom centered m55010 pointed\" onClick=mem_not_delm('shc$M5','hc$M5')>
    ...
    if($red=="on"){
        ...
        <div id=\"rbc0$M5\" style=\"display: none;\">
            <textarea id=\"rcm$M5\" rows=3 style=\"width: 95%;\"
             onFocus=clearsq('rcm$M5','rbc$M5');>$M11</textarea>
            <div id=\"rbc$M5\" style=\"display: none;\">
                <table><tr><td class=\"fcomblue\" width=100>
                    <ul class=\"intop\"><li><a onclick=\"comm_red('$idrec','rcm$M5','in$M5')\">$Ed</a></li></ul>
                </td></tr></table>
            </div>
        </div><br />";
    }
