...
function mem_add(id) {
    ...
    $.ajax({
        ...
        cache: false,
        success: function (data) {
            // Раніше тут перевірявся підрядок "avatar/s" як ознака успіху — але реальний
            // аватар користувача рендериться як "/storage/avatar/N.jpg" (без "s"), тому ця
            // перевірка ніколи не спрацьовувала і поле не очищалось. Тепер — за тим самим
            // маркером помилки "s235*64@75", що й в інших формах на сайті (askc, comm_add...).
            var nseek = data.indexOf("235*64@75");
            if (nseek < 0) {
...
function setHomeBlockCookie(key, val) {
    var exp = new Date();
...
function showHomeBlock(key) {
    ...
    // рендеряться без src (лише data-src), щоб браузер їх взагалі не тягнув,
    // поки блок не розкрито — підставляємо src лише зараз, при показі.
...
function mem_addp(id) {
    ...
    $.ajax({
        ...
        cache: false,
        success: function success(data) {
            // Той самий фікс, що й у mem_add() — перевіряємо маркер помилки, а не підрядок
            // "avatar/s", якого ніколи не буває в реальній відповіді.
            var nseek = data.indexOf("235*64@75");
            if (nseek < 0) {
                document.getElementById("memt_res").innerHTML = data;
                var txt = document.getElementById("memt");
