        ...
        if (Auth::user()) {
            ...
            if ($nresultfr>0){
                ...
                $affected = DB::table('Friends')
                    ->where('Num2', $Numm)
                    ->where('Num1', $Numfr)
                    ->update(['Argue' => 2, 'Im2' => $Imm2, 'Priz2' => $Prizm2, 'avatar2' => $avatar2, 'avy2' => $avy2]);
    ...
    public function weather(Request $request)
    {
		...
		function weatherForFive($x, $y) {
            ...
            for ($n = 1; $n <= $count_news; $n++) {
                ...
                $tempmax = (int)$tmax - 273.15;

                $avertemp = round(($tempmax+$tempmin)/2);
                ...
                $start = "speed";
                $position = strpos($contentpol, $start);
                $speed = substr($contentpol, $position + 7);

                $finish = ",";
                $position = strpos($speed, $finish);
                $speed = round(substr($speed, 0, $position),1);

                $start = "deg";
                $position = strpos($contentpol, $start);
                $deg = substr($contentpol, $position + 5);

                $finish = ",";
                $position = strpos($deg, $finish);
                $deg = round(substr($deg, 0, $position),1)+317;
                ...
                if($date==$todayDate){$dayOfWeek=__('messages.today');}
                    ...
                    $numbdayOfWeek = date("N", strtotime($date));
                    $day_e = "messages.day$numbdayOfWeek";
                    $dayOfWeek = __($day_e);
                }

                $hours = substr($time, 11, 5);
                if($historyday != $dayOfWeek){echo "<h3>$dayOfWeek $day $monm </h3>";}
                $historyday = $dayOfWeek;


                $svg = '<svg class="media__icon" width="15" height="15"><use href="/images/icons.svg#icon-compass"></use></svg>';
                $newdir= "<svg transform=\"rotate($deg)\"";
                $rotatedSvg = str_replace('<svg', $newdir, $svg);

                if($hours=="00:00"){$color=0.55;}
                if($hours=="03:00"){$color=0.45;}
                if($hours=="06:00" || $hours=="21:00"){$color=0.35;}
                if($hours=="09:00" || $hours=="18:00"){$color=0.25;}
                if($hours=="15:00"){$color=0.20;}
                if($hours=="12:00"){$color=0.15;}
                $speed_e = __('messages.speed');
                echo"<table border='0' style=\"background-color: rgba(167, 174, 212, $color)\">
                <tr valign='center'>
                    <td style=\"width: 20%;\"><div id='weather__wind'>$hours</div></td>
                    <td style=\"width: 23%; align: center; \" ><div id='weather__degree_future'> $avertemp&#186;</div></td>
                    <td style=\"width: 24%;\"> <img src=\"https://openweathermap.org/img/wn/$icon.png\";></td>
                    <td style=\"width: 33%;\" align='center'><div id='weather__wind'>$rotatedSvg<br /> $speed $speed_e</div></td>
                </tr>
                </table>";
